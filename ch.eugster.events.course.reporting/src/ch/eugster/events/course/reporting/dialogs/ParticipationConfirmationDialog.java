...
public class ParticipationConfirmationDialog extends TitleAreaDialog
{
	...
	protected Control createDialogArea(final Composite parent)
	{
		...
		if (User.getCurrent() != null)
		{
			this.userPropertyTemplatePath = User.getCurrent().getProperty(
					UserProperty.Property.PARTICIPATION_CONFIRMATION_TEMPLATE_PATH.key());
			if (this.userPropertyTemplatePath == null)
			{
				this.userPropertyTemplatePath = UserProperty.newInstance(User.getCurrent());
				this.userPropertyTemplatePath.setKey(UserProperty.Property.PARTICIPATION_CONFIRMATION_TEMPLATE_PATH
						.key());
		...
		documentSelector.addSelectionListener(new SelectionAdapter()
		{
			...
			{
				String path = ParticipationConfirmationDialog.this.documentPath.getText();
				FileDialog dialog = new FileDialog(ParticipationConfirmationDialog.this.getShell());
				...
				if (path != null)
				{
					ParticipationConfirmationDialog.this.documentPath.setText(path);

				}
				File file = new File(ParticipationConfirmationDialog.this.documentPath.getText());
				if (file.exists())
				{
					userPropertyTemplatePath.setValue(file.getAbsolutePath());
				}
				ParticipationConfirmationDialog.this.getButton(IDialogConstants.OK_ID).setEnabled(file.isFile());
			}
		});
