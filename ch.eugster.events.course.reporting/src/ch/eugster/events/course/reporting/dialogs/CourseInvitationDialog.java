	...
	private void buildDocument(final Collection<DataMap> dataMaps)
	{
		if (dataMaps.size() == 0)
		{
			MessageDialog dialog = new MessageDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
					MSG_TITLE_NO_COURSES, null, "Es sind keine auswertbaren Daten ausgewählt.",
					...
					if (!status.isOK())
					{
						if (status.getSeverity() == IStatus.CANCEL)
						{
							MessageDialog dialog = new MessageDialog(PlatformUI.getWorkbench()
									.getActiveWorkbenchWindow().getShell(), MSG_TITLE_NO_COURSES, null, status
						...
						else
						{
							ErrorDialog dialog = new ErrorDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
									.getShell(), MSG_TITLE_NO_COURSES, status.getMessage(), status, 0);
							dialog.open();
						}
					}
				}
