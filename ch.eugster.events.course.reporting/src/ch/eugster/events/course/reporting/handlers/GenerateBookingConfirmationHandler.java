...
import ch.eugster.events.course.reporting.dialogs.BookingConfirmationDialog;
import ch.eugster.events.documents.services.DocumentBuilderService;
import ch.eugster.events.persistence.model.Booking;
import ch.eugster.events.persistence.model.Course;

public class GenerateBookingConfirmationHandler extends AbstractHandler implements IHandler
{
	@Override
	public Object execute(final ExecutionEvent event) throws ExecutionException
	{
		if (event.getApplicationContext() instanceof EvaluationContext)
		{
			...
			if (context.getParent().getVariable("selection") instanceof StructuredSelection)
			{
				...
				BookingConfirmationDialog dialog = new BookingConfirmationDialog(shell, ssel);
				dialog.open();
			}
		}
