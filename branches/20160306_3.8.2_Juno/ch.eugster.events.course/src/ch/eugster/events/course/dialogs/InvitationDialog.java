...
public class InvitationDialog extends TitleAreaDialog
{
	...
	private static final String DIALOG_TITLE = "Vorlage Einladung";

	private boolean isPageComplete = false;

	public InvitationDialog(final Shell parentShell, final StructuredSelection selection)
	{
		super(parentShell);
	...
	protected Control createDialogArea(final Composite parent)
	{
		...
		documentSelector.addSelectionListener(new SelectionAdapter()
		{
			...
			{
				String path = InvitationDialog.this.documentPath.getText();
				FileDialog dialog = new FileDialog(InvitationDialog.this.getShell());
				...
				if (path != null)
				{
					InvitationDialog.this.documentPath.setText(path);

				}
				File file = new File(InvitationDialog.this.documentPath.getText());
				if (file.exists())
				{
					userPropertyTemplatePath.setValue(file.getAbsolutePath());
				}
				InvitationDialog.this.getButton(IDialogConstants.OK_ID).setEnabled(file.isFile());
			}
		});
