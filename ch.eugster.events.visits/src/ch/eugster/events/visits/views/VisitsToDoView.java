...
{
	private static final String ID = "ch.eugster.events.visits.todo.view";
			...
			public void update(ViewerCell cell)
			{
				...
				if (object instanceof Visit)
				{
					...
					else
					{
						...
						{
							Collection<LinkPersonAddress> links = visit.getTeacher().getLink().getPerson().getLinks();
							for (LinkPersonAddress link : links)
							{
								if (link.getAddressType().getId()
										.equals(Activator.getDefault().getSettings().getDefaultAddressType().getId()))
								{
						...
						if (Activator.getDefault().getSettings().getDefaultAddressType() != null)
						{
							...
							for (LinkPersonAddress link : links)
							{
								if (link.getAddressType().getId()
										.equals(Activator.getDefault().getSettings().getDefaultAddressType().getId()))
								{
									cell.setText(link.getAddress().getProvince());
								}
							}
						}
						cell.setBackground(getBackground(visit.getTheme()));
