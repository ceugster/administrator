...
public class AddCourseGuideHandler extends AbstractHandler implements IHandler
{
		...
		if (activePart instanceof ContentOutline)
		{
			...
			if (contentOutline.getCurrentPage() instanceof CourseEditorContentOutlinePage)
			{
				...
				if (context.getParent().getVariable("selection") instanceof StructuredSelection)
				{
					...
					if (!ssel.isEmpty() && ssel.getFirstElement() instanceof CourseEditorContentOutlinePage.CourseGuideGroup)
					{
						CourseGuideGroup courseGuideGroup = (CourseGuideGroup) ssel.getFirstElement();
						CourseGuide courseGuide = CourseGuide.newInstance(courseGuideGroup.getRoot().getCourse());
