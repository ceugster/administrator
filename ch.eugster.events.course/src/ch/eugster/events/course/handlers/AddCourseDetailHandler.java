...
public class AddCourseDetailHandler extends AbstractHandler implements IHandler
{
		...
		if (activePart instanceof ContentOutline)
		{
			...
			if (contentOutline.getCurrentPage() instanceof CourseEditorContentOutlinePage)
			{
				...
				if (context.getParent().getVariable("selection") instanceof StructuredSelection)
				{
					...
					if (!ssel.isEmpty() && ssel.getFirstElement() instanceof CourseEditorContentOutlinePage.CourseDetailGroup)
					{
						CourseDetailGroup courseDetailGroup = (CourseDetailGroup) ssel.getFirstElement();
						CourseDetail courseDetail = CourseDetail.newInstance(courseDetailGroup.getRoot().getCourse());
