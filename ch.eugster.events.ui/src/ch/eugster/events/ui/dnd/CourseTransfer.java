...
public class CourseTransfer extends ByteArrayTransfer
{
	...
	public static final String TYPE_NAME = "course-selection-transfer-format" + (new Long(System.currentTimeMillis())).toString(); //$NON-NLS-1$;

	public static final int TYPEID = Transfer.registerType(CourseTransfer.TYPE_NAME);
	 ...
	 */
	public static CourseTransfer getTransfer()
	{
		return CourseTransfer.INSTANCE;
	}

	/**
	 * Only the singleton instance of this class may be used.
	 */
	protected CourseTransfer()
	{
	}
	...
	{
		return !(result instanceof byte[]) || !CourseTransfer.TYPE_NAME.equals(new String((byte[]) result));
	}
	...
	{
		byte[] check = CourseTransfer.TYPE_NAME.getBytes();
