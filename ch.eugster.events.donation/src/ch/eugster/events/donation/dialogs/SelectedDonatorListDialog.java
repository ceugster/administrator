...
public class SelectedDonatorListDialog extends TitleAreaDialog
{
	...
	public SelectedDonatorListDialog(final Shell parentShell, final StructuredSelection selection)
	{
		super(parentShell);
	...
	private void computeDonation(final Map<String, DataMap<?>> maps, final Donation donation)
	{
		if (!donation.isDeleted())
		{
			if (donation.getLink() == null || !donation.getLink().isValid())
			{
				maps.put("A" + donation.getAddress().getId(), new AddressMap(donation.getAddress()));
			}
			else if (donation.getLink().isValid())
			{
				maps.put("P" + donation.getLink().getId(), new LinkMap(donation.getLink()));
			}
		}
	}

	private void computeDonationYear(final Map<String, DataMap<?>> maps, final DonationYear year)
	{
		if (!year.isDeleted())
		{
	...
	private DataMap<?>[] createDataMaps(final StructuredSelection ssel)
	{
		...
		return maps.values().toArray(new DataMap<?>[0]);
	}
